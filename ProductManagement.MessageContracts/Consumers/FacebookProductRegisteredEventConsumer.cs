...
{
    public class FacebookProductRegisteredEventConsumer : IConsumer<IProductRegisteredEvent>
    {
        public async Task Consume(ConsumeContext<IProductRegisteredEvent> context)
        {
            Console.WriteLine($"{context.Message.ProductName} İsimli Ürün Facebookda Yayınlandı.");
        }
    }
}